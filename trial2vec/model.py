...
class Trial2Vec(TrialSearchBase):
    ...
    def load_model(self, checkpoint):
        ...
        state_dict = torch.load(checkpoint_filename,map_location=torch.device("cpu"),weights_only=False)
    ...
    def _wrap_model(self, model, device):
        # Updated to honor 'mps' device:
        device_str = str(device).lower()
        if isinstance(device, list):
            # if the user provided a list of device IDs, assume CUDA data parallel
            ...
            # Fallback to CPU if something else is passed
            model.to(torch.device('cpu'))
        return model
